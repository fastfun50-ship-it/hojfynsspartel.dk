...
(async () => {
  ...
  const shots = [
    { name: 'hero', path: '/', sel: null, scroll: 0 },
  ...
  for (const w of [375, 1280]) {
    ...
    for (const s of shots) {
      const url = 'http://127.0.0.1:3000' + s.path + (s.hash || '');
      await page.goto(url, { waitUntil: 'networkidle0', timeout: 60000 });
      if (s.hash) {
        await page.evaluate((h) => { const el = document.querySelector(h); if (el) el.scrollIntoView(); }, s.hash);
